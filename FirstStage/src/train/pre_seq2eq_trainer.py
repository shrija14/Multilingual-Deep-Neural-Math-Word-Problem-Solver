...
class SupervisedTrainer(object):
    ...
    def _train_batch(self, input_variables, input_lengths,target_variables, target_lengths, model,\
                         template_flag, teacher_forcing_ratio, mode, batch_size):
        decoder_outputs, decoder_hidden, symbols_list, att_list = \
    ...
    def _train_epoches(self, data_loader, model, batch_size, start_epoch, start_step, n_epoch, \
                            mode, template_flag, teacher_forcing_ratio):
        print_loss_total = 0
        ...
        for epoch in range(start_epoch, n_epoch + 1):
            ...
            #check_idx = int(train_ans_acc*100/10)
            if valid_temp_acc >= max_valid_temp_acc:
                max_valid_temp_acc = valid_temp_acc
                ...
                #                              save_according_name("./experiment", 'best_'+str(test_temp_acc))

            print("Epoch: %d, Step: %d, train_acc: %.2f, validate_acc: %.2f, test_acc: %.2f"\
                  % (epoch, step, train_temp_acc, valid_temp_acc,  test_temp_acc ))


    def train(self, model, data_loader, batch_size, n_epoch, template_flag, \
                        resume=False, optimizer=None, mode=0, teacher_forcing_ratio=0):
