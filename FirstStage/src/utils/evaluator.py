...
class Evaluator(object):
    ...
    def get_op_template(self, seq_var):
        equ_list = []
        for idx in seq_var.data.cpu().numpy():
            if idx == self.pad_in_classes_idx:
                break
    ...
    def evaluate(self, model, data_loader, data_list, template_flag, batch_size, \
                      evaluate_type, use_rule, mode, filename_suffix):
        ...
        for batch_data_dict in batch_generator:
            ...
            for i in range(batch_size):
                ...
                #    print 
                #print target_equ
                gen_equ = self.get_op_template(seq_var[i])
                pg_total_list.append([batch_index[i], gen_equ])
                '''
                gen_ans = self.compute_gen_ans(seq_var[i], batch_num_list[i])
                ...
                #pdb.set_trace()
                pg_total_list.append(dict({'index': batch_index[i], 'gen_equ': gen_equ, 'att':batch_att[i], \
        ...
        #pdb.set_trace()
        filename = "../data/pg_seq_norm_0821_" + filename_suffix + ".json" 
