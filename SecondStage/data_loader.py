...
class DataLoader:
    def __init__(self, dataset):#, dataset, math23k_vocab_list, math23k_decode_list):
        ...
        math23k_vocab_list = ['PAD','<S>', 'EOS', '1', 'PI', '100']
        math23k_decode_list = ['temp_0','temp_1','temp_2','temp_3','temp_4','temp_5','temp_6','temp_7',\
				...
				'1', 'PI', 'PAD', '<S>','EOS', '100']
        math23k_vocab_list = math23k_decode_list

        #math23k_decode_list = ['temp_0','temp_1','temp_2','temp_3','1', 'PI', 'PAD', '<S>','EOS']
        for k, v in dataset.items():
            for elem in v['text'].split(' '):
                if elem not in math23k_vocab_list:
                    math23k_vocab_list.append(elem)
        
        with open('./data/vocabEmbed.pkl', 'rb') as f:
            dt = pickle.load(f)
        
        self.embed_list = []
        for v in math23k_vocab_list:
            if v in dt:
                self.embed_list.append(dt[v])
            else:
                self.embed_list.append(np.random.normal(scale=0.6, size=(512, )).tolist())
        ...
        #     self.decode_emb_idx = [self.vocab_dict[elem] for elem in self.decode_classes_list]
        # except:
        #     print(self.decode_classes_list)
        #     print("lol")
