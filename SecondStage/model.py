...
class RecursiveNN(nn.Module):
	...
	def getLoss_train(self, tree_node, num_embed, look_up):
		...
		# predictions = torch.randint(4, self.labelList.size()).to(device)
		# print(predictions)
	...
	def test_forward(self, tree_node, num_embed, look_up):
		...
		acc_elemwise, acc_t = self.compute_acc_elemwise(predictions, self.labelList)
		acc_integrate = self.compute_acc_integrate(predictions, self.labelList)
		return loss, predictions, acc_elemwise, acc_t, acc_integrate#, tree_node
...
class Self_ATT_RTree(nn.Module):
	def __init__(self, data_loader, encode_params, RecursiveNN):
		...
		pretrained_embeddings = torch.FloatTensor(self.data_loader.embed_list).to(device) # to(device) ?
		self.embed_model = nn.Embedding.from_pretrained(pretrained_embeddings, freeze=False)
	...
	def _prepare_for_recur(self):
		self.fixed_num_symbol = ['1', 'PI', '100']
	...
	def _prepare_for_pointer(self):
		self.fixed_p_num_symbol = ['EOS','1', 'PI', '100']
	...
	def forward(self, input_tensor, input_lengths, num_pos, b_gd_tree):
		...
		for b_i in range(batch_size):
			...
			look_up = self.fixed_num_symbol + ['temp_'+alphas[temp_i] for temp_i in range(len(num_pos[b_i]))]
	...
	def test_forward_recur(self, input_tensor, input_lengths, num_pos, b_gd_tree):
		...
		for b_i in range(batch_size):
			...
			#self.recur_nn(gd_tree_node, en_output, look_up)
			l,p, acc_e, acc_e_t, acc_i = self.recur_nn.test_forward(gd_tree_node, en_output, look_up)
			#print (p)tensor([ 0,  2])
			#print (l)tensor(1.5615)
			#print (l)
			#print (post_order(gd_tree_node))
		...
		#print (torch.sum(torch.cat(batch_loss_l)))
		#print ()
		return batch_loss_l,batch_predictions, batch_count, batch_acc_e_list, batch_acc_e_t_list, batch_acc_i_list
