...
def duplicate(entry):
	...
	for i in post_temp:
		if 'temp' in i:
			num = dup_num_list[int(alphas.index(i.split('_')[1]))]
			post_equ.append(num)
		else:
			post_equ.append(i)
	try:
...
def main():
	# dolphin = read_json('final_dolphin_data_n.json')
	dolphin = read_json('final_combined.json')
	dolphin = json.loads(dolphin)
	n_repeats = 0
  	# n_repeats = 0
...
#		print(new_dolphin)
	...
	print(len(dolphin.keys()),len(new_dolphin.keys()))
	
	write_json('final_combined_replicate.json', new_dolphin)
	...
	print(len(index)-valid_size, valid_size)

	# validation ids
	write_json('valid_ids_combined_s2.json', valid_ids.tolist())
	# test ids
	# fp1 = open('./data/id_ans_test_math23k', 'w')
	...
	# write_json('pg_norm_test_math23k.json', testtemp)
